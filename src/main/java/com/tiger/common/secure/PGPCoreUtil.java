...
public class PGPCoreUtil {
    ...
    static byte[] compressFile(InputStream inputStream, int algorithm) throws IOException {
        ByteArrayOutputStream bOut = new ByteArrayOutputStream();
        PGPCompressedDataGenerator comData = new PGPCompressedDataGenerator(algorithm);
        writeFileToLiteralData(comData.open(bOut), inputStream);
        comData.close();
        return bOut.toByteArray();
    }

    static void writeFileToLiteralData(OutputStream out, InputStream inputStream) throws IOException {
        PGPLiteralDataGenerator pgpLiteralDataGenerator = new PGPLiteralDataGenerator();
        try {
            byte[] buffer = IOUtils.toByteArray(inputStream);
            try (OutputStream os = pgpLiteralDataGenerator.open(out, PGPLiteralData.BINARY, "",buffer.length, new Date())) {
                os.write(buffer);
            }
        } finally {
